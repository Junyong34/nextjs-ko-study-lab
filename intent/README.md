...
| `visualize-cache-shopping-story` | 쇼핑몰 시나리오(유준/서아) 내러티브로 감싼 Cache Components 시각화, `cache-shell`/`cache-keys`/`cache-lifetime`/`cache-tags` 4종 재사용 | `nextjs-app/packages/ui/src/reference-visualize/toolkit/examples/showcase-cache-demos.tsx`, `showcase-types.ts`, `examples/cache-shopping-story/` (신규) | done | [`intent.md`](./visualize-cache-shopping-story/intent.md) | [`intent.md#requirements`](./visualize-cache-shopping-story/intent.md#requirements) | [`plan.md`](./visualize-cache-shopping-story/plan.md) | 구현·타입·빌드·수동(Chromium) 검증 완료. 기존 4개 데모 파일은 무수정. PR 머지 전 승인 효력 없음 |
