...
| `visualize-cache-shopping-story` | 쇼핑몰 시나리오(유준/서아) 내러티브로 감싼 Cache Components 시각화, `cache-shell`/`cache-keys`/`cache-lifetime`/`cache-tags` 4종 재사용 | `nextjs-app/packages/ui/src/reference-visualize/toolkit/examples/showcase-cache-demos.tsx`, `showcase-types.ts`, `examples/cache-shopping-story/` (신규) | draft | [`intent.md`](./visualize-cache-shopping-story/intent.md) | [`intent.md#requirements`](./visualize-cache-shopping-story/intent.md#requirements) | [`plan.md`](./visualize-cache-shopping-story/plan.md) | intent 대화 승인(Open questions 기본값 채택). plan 승인 대기, 구현 미착수 |
