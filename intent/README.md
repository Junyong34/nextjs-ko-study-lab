...
| `visualize-cache-shopping-story` | 쇼핑몰 시나리오(유준/서아) 내러티브로 감싼 Cache Components 시각화, `cache-shell`/`cache-keys`/`cache-lifetime`/`cache-tags` 4종 재사용 | `nextjs-app/packages/ui/src/reference-visualize/toolkit/examples/showcase-cache-demos.tsx`, `showcase-types.ts`, `nextjs-app/apps/shell/src/app/visualize/`, `nextjs-app/apps/shell/src/components/visualize/` | draft | [`intent.md`](./visualize-cache-shopping-story/intent.md) | [`intent.md#requirements`](./visualize-cache-shopping-story/intent.md#requirements) | 미정 | intent 대화 초안. PO(사용자) 승인 전, Open questions(라우팅·슬러그) 미결 |
