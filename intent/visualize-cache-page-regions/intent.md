...
Author: Claude
Status: done
Approval: 2026-09-19 대화에서 사용자가 직접 요청("페이지가 영역별로 분리돼 있고, 영역별 컴포넌트마다 캐시 설정에 따라 데이터를 어떻게 가져와 사용자에게 보여주는지 시각화 — 캐시 설정된 컴포넌트, 함수, 캐시 없이 매번 새로 가져오는 영역 등을 분리해서"). 저장소 규칙상 PR 머지 전 승인 효력 없음. 선례(`visualize-cache-mall-story`)와 같이 대화 승인으로 구현을 진행하며, 요구사항·설계·완료 기준을 이 문서에 통합한다(작은 범위 변경 양식). 2026-09-24 사용자 지시로 done 전환 — 구현 `6728970` main 반영 확인, 이후 셸 production build(GA 작업 `5957095` 검증, 834개 정적 페이지) 통과.
...
- [x] `/visualize`에 "cache-regions" 카드가 노출되고 `/visualize/cache-regions`가 동작한다.
- [x] 캐시 채워짐 시나리오에서 헤더·상품 목록이 첫 페인트와 함께 도착하고, 환율 배너는 쿠키를 읽은 뒤 `getRate` 캐시 HIT로(외부 API 호출 없이), 추천은 DB까지 갔다가 가장 늦게 스트리밍되어 채워진다.
- [x] 캐시 비어 있음 시나리오에서 상품 목록이 DB → 캐시 저장을 거치며 첫 페인트가 늦어지고, 환율 배너는 외부 API 호출 후 캐시에 저장되는 것이 보인다.
- [x] 노트·코드 지도 호버 시 캔버스 해당 행·페이지 영역이 강조되고, 반대로 캔버스 호버도 노트에 반영된다.
- [x] 라이트/모바일 폭에서 깨지지 않으며(캔버스는 다른 데모처럼 최소 폭 가로 스크롤), 타입체크 통과, 파일당 250줄 이하.
