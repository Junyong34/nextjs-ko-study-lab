...
학습 순서 자체는 아직 정하지 않았습니다. 다만 현재 241개 데모를 찾기 위한 색인 UX의 구현 범위와 기술 방향은 [Demo Index Discovery UX 구현 계획](../../docs/plans/2026-08-22-2219-feat-demo-index-discovery-plan.md)으로 확정했습니다.

계획에는 검색 우선 탐색, 기존 문서 대분류 필터, 3/2/1 반응형 압축 카드, 페이지당 24개 페이지네이션, 모바일 이전·현재·다음 조작, URL query 상태, 상세 복귀와 스크롤 복원이 포함됩니다.

**남은 것**: 위 계획에 따른 실제 코드 구현과 브라우저 QA입니다. 학습 순서 기반 정렬, 무한 스크롤, virtualization, 새 metadata 태그와 별도 초기화 링크는 이번 구현 범위에 포함하지 않습니다.
...
- **구현 계획:** [Demo Index Discovery UX 구현 계획](../../docs/plans/2026-08-22-2219-feat-demo-index-discovery-plan.md)
- **대상:** 셸의 `/demo` 색인, 데모 카드·페이지네이션 UI, 상세 페이지의 목록 복귀와 스크롤 복원
- **상태:** 계획 문서 작성 완료. 실제 코드 구현과 viewport별 브라우저 검증이 남아 있습니다.
