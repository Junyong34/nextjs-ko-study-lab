        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400 max-w-md mx-auto break-keep">
          {query && category && category !== 'All' && category !== '전체' ? (
            <>
              ...
              <span className="font-semibold text-zinc-800 dark:text-zinc-200">"{category}"</span> 카테고리와 일치하는 데모를 찾을 수 없습니다.
            </>
          ) : query ? (
            <>
              <span className="font-semibold text-zinc-800 dark:text-zinc-200">"{query}"</span> 검색어와 일치하는 데모를 찾을 수 없습니다.
            </>
          ) : category && category !== 'All' && category !== '전체' ? (
            <>
              <span className="font-semibold text-zinc-800 dark:text-zinc-200">"{category}"</span> 카테고리에 해당하는 데모가 없습니다.
            </>
          ) : (
            '검색 조건을 만족하는 데모가 없습니다. 검색어를 변경하거나 필터를 초기화해 보세요.'
          )}
        </p>
