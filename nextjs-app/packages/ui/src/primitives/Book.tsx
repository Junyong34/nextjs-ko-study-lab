...
export function Book({ children, coverClassName = DEFAULT_COVER, depth, width }: BookProps) {
  return (
    ...
    >
      <div className="contain-inline-size aspect-[49/60] w-fit relative [transform-style:preserve-3d] min-w-[calc(var(--book-width))] transition-transform duration-500 ease-out book-interactive-cover group-hover:[transform:rotateY(-20deg)_scale(1.066)_translateX(-8px)] group-active:[transform:rotateY(-20deg)_scale(1.066)_translateX(-8px)] hover:[transform:rotateY(-20deg)_scale(1.066)_translateX(-8px)] active:[transform:rotateY(-20deg)_scale(1.066)_translateX(-8px)]">
        <Stack
          align="stretch"
          className={cn(
