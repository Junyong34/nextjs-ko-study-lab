...
export interface M4DemoAuditDetail {
  id: string
  url: string
  zone: string
  doc: string
...
function extractInteractiveUiElements(content: string) {
  ...
  const textElemRegex =
    /<(?:span|strong|p|h\d|div)[^>]*class(?:Name)?=["'][^"']*(?:font-|text-|badge|tab|pill|btn)[^"']*["'][^>]*>([^<]+)<\/(?:span|strong|p|h\d|div)>/gi
...
export function runM4AdversarialUiLabelsAudit() {
  console.log('============================================================')
  console.log('   CHALLENGER 1: Milestone M4 UI Label & Interaction Audit  ')
  console.log('   Scope: 2-guides (77) & 5-architecture (4) — 81 Demos Total')
  console.log('============================================================\n')

  const manifest = loadDemosManifest()
  const m4Demos = getM4Demos(manifest)

  console.log(`Found ${m4Demos.length} M4 demos in manifest (Expected: 81).\n`)

  let passCount = 0
  let failCount = 0
  let warnCount = 0
  const findings: { demo: string; issue: string; severity: 'ERROR' | 'WARN' }[] = []
  const auditDetails: M4DemoAuditDetail[] = []
  ...
  for (const demo of m4Demos) {
    ...
    const { labels: extractedUiLabels, ternaries: extractedTernaries } =
      extractInteractiveUiElements(fullClean)
    ...
    const leakMatches = guideRawText.match(/(\$\{[^}]+\}|&(?:lt|gt|amp|quot|apos|#39|#123|#125);)/gi)
    if (leakMatches) {
      leaks.push(...leakMatches)
      issues.push(`String leak detected in guide: ${leakMatches.join(', ')}`)
      findings.push({
        demo: demo.url,
        issue: `String leak in guide: ${leakMatches.join(', ')}`,
        severity: 'ERROR',
      })
    }
    ...
    guide.steps.forEach((s) => {
      ...
      brackets.forEach((b) => {
        // Check concatenated ternary artifacts e.g. [처리 중... 로그아웃]
        if (
          b.includes('...') &&
          b.includes(' ') &&
          (b.includes('검증') ||
    ...
    // 3. Check observe & observeAt in last step
    const lastStep = guide.steps[guide.steps.length - 1]
    const missingObserve =
      !lastStep || !lastStep.observe || lastStep.observe.trim().length < 5
    ...
    if (
      observeAt &&
      !['playground', 'verification', 'devtools', 'network', 'console'].includes(observeAt)
    ) {
    ...
    const hasErrors = issues.some((iss) => !iss.startsWith('WARN') && !iss.includes('Concatenated ternary'))
    const status: 'PASS' | 'WARN' | 'FAIL' = hasErrors
      ? 'FAIL'
      : (missingOrMismatchedLabels.length > 0 || concatenatedLabels.length > 0)
        ? 'WARN'
        : 'PASS'
    ...
    const norm = (demo.doc || '').replace(/^\/?(nextjs-docs\/)?/, '')
    const category = norm.split('/')[0] || ''
  ...
  console.log('------------------------------------------------------------')
  console.log(`Audited: ${m4Demos.length} demos (2-guides: 77, 5-architecture: 4)`)
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const result = runM4AdversarialUiLabelsAudit()
