...
export function VerificationFooter(props: VerificationFooterProps = {}) {
  ...
  const defaultExpected =
    '• next/image 컴포넌트를 통해 최적화 파이프라인이 동작하고, quality/preload 옵션에 따라 최적화된 이미지 URL 쿼리와 종횡비 고정이 적용됨\n• 일반 <img> 태그 대비 WebP/AVIF 자동 변환 및 Zero CLS 레이아웃 예약 확인\n• preload 선언 시 LCP 사전 로드(<link rel="preload">) 연동 검증 결과'

  let defaultActual = '• 인터랙션 대기 중 (옵션 툴바에서 퀄리티 슬라이더와 preload 체크박스를 조작해보세요)'
  if (hasInteracted) {
    defaultActual = `• 이미지 최적화 파이프라인: next/image 활성화 (WebP/AVIF 자동 변환 지원)\n• 적용 옵션: quality=${quality}%, preload=${
      preload ? 'true (LCP 사전 로드 활성화)' : 'false (지연 로딩 loading="lazy")'
    }\n• 뷰 모드: ${
      viewMode === 'both'
      ...
      <DemoDeepDiveCard title="next/image 자동 WebP/AVIF 최적화 & Zero CLS 이미지 로딩">
        <div className="space-y-3.5 text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
          <div>
            ...
            <p>
              <code>next/image</code> 컴포넌트는 클라이언트 브라우저가 지원하는 최신 포맷(WebP/AVIF)으로 온디맨드 자동 변환하고, <code>width</code>와 <code>height</code>를 기반으로 종횡비를 미리 예약하여 레이아웃 이동(CLS)을 방지하며, <code>preload</code> 속성을 통해 LCP(Largest Contentful Paint) 이미지를 사전 로드하는 Next.js 공식 표준 최적화 스펙입니다.
            </p>
          </div>
          ...
          <div>
            ...
            <p>
              본 데모에서는 일반 <code>&lt;img&gt;</code> 태그와 <code>next/image</code> 컴포넌트를 나란히 배치하고, 퀄리티 슬라이더(quality) 및 <code>preload</code> 체크박스를 조작할 때 생성되는 <code>/_next/image?url=...&q=...</code> 최적화 쿼리 스트링과 이미지 파이프라인의 실시간 변화를 대조 검증합니다.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-zinc-600 dark:text-zinc-400 pl-1">
              ...
              <li><strong>LCP 로딩 시간 단축</strong>: <code>preload</code> 지정 시 <code>&lt;link rel="preload"&gt;</code>를 헤더에 주입하여 첫 화면 히어로 이미지를 즉시 로드합니다.</li>
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-zinc-600 dark:text-zinc-400 pl-1">
              <li>쇼핑몰 메인 히어로 배너 및 프로모션 대표 이미지 (<code>preload=true</code> 적용)</li>
