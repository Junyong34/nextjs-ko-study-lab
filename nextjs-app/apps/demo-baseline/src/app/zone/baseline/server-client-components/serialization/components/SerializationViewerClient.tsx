...
(Symbol properties are omitted from JSON payload)`,
  ...
  return (
    <div className="space-y-6">
      ...
      <DemoPlaygroundCard title="RSC Props 직렬화 인스펙터 & 유효성 시뮬레이터" className="space-y-6">
        {/* 1. 서버에서 수신된 Props 런타임 직렬화 검증 패널 */}
        <div className="space-y-3.5 rounded-2xl border-2 border-emerald-500/50 bg-emerald-50/20 p-5 dark:border-emerald-700/60 dark:bg-emerald-950/20 shadow-xs">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-emerald-200/80 pb-3 dark:border-emerald-800/80">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-sm font-extrabold text-zinc-900 dark:text-white">
                RSC 수신 Props 런타임 직렬화 검증기
              </span>
            </div>

            {/* 실시간 런타임 검증 뱃지 (하드코딩 아님: JSON.stringify 검증 결과) */}
            <div className="flex items-center gap-2">
          ...
          {activeViewTab === 'structured' ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 pt-1">
              <div className="rounded-xl border border-emerald-200/80 bg-white p-4 dark:border-emerald-900/60 dark:bg-zinc-900 shadow-2xs space-y-1.5">
                <span className="text-xs font-bold text-emerald-900 dark:text-emerald-300">
                  1. 기본 원시값 (Primitives):
                </span>
                <div className="font-mono text-xs space-y-1 text-zinc-800 dark:text-zinc-200 pt-1">
        ...
        {/* 2. [인터랙티브] 타입별 직렬화 가능 vs 불가능 대조 시뮬레이터 */}
        <div className="space-y-3.5 rounded-2xl border border-zinc-200 bg-zinc-50/90 p-5 dark:border-zinc-800 dark:bg-zinc-900/60 shadow-xs">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200/80 pb-3 dark:border-zinc-800">
            <div>
              <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100">
                타입별 직렬화 가능(Serializable) vs 불가능(Non-Serializable) 대조 시뮬레이터
              </h4>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {(Object.keys(simulationMap) as SimulationType[]).map((key) => {
              ...
              return (
                ...
                >
                  <span className={`text-[10px] font-bold font-mono px-1.5 py-0.2 rounded mb-1 ${
          ...
          {/* 선택된 타입 상세 분석 카드 */}
          <div className={`space-y-3 rounded-xl border p-4 shadow-2xs ${
            currentSim.isError
