...
export function PrefetchController() {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-zinc-200 bg-zinc-50 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50">
      {/* 상태 표시 바 */}
      <div className="flex flex-wrap items-center justify-between gap-2 rounded border border-zinc-300 bg-white px-3 py-2 text-xs shadow-2xs dark:border-zinc-700 dark:bg-zinc-950">
        <div className="flex items-center gap-2">
          <span className="font-mono text-zinc-400">Current URL:</span>
          <span className="font-mono font-bold text-zinc-900 dark:text-zinc-100 bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-300 dark:border-zinc-700">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <button
          ...
          className="inline-flex items-center rounded border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400 cursor-pointer"
        >
          대시보드 복귀
        </button>
      </div>
      ...
      <div className="text-[11px] text-zinc-500 font-mono">
        • 최근 실행 액션: <span className="font-medium text-zinc-800 dark:text-zinc-200">{lastAction}</span>
      </div>
    </div>
  )
}
