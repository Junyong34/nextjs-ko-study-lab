...
export function VerificationFooter(props: VerificationFooterProps = {}) {
  ...
  const defaultExpected = "• generateMetadata 동적 메타데이터 & 소셜 공유 미리보기 사양에 따른 정상 동작 및 상태 변화 관찰"
      ...
      />
              ...
              <div className="space-y-3.5 text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
                <div>
                  ...
                  <p>Next.js App Router의 <code>export const metadata</code> 정적 객체 및 <code>export async function generateMetadata({'{'} params, searchParams {'}'})</code> 함수는 페이지별 HTML <code>{'<'}head{'>'}</code> 메타 태그(Title, Description, OpenGraph, Canonical)를 서버에서 동적으로 생성하고, 동일 렌더 사이클 내의 <code>fetch</code> 요청을 자동 중복 제거(Deduping)하는 표준 SEO 스펙입니다.</p>
                </div>

                <div>
                  <h5 className="font-bold text-zinc-900 dark:text-zinc-100 mb-1">2. 데모 예제 기반 동작 원리</h5>
                  <p>본 데모에서는 URL 세그먼트 파라미터(<code>[id]</code>)를 받아 백엔드 상품 정보를 조회하고, 상품명과 가격을 결합한 동적 <code>{'<'}title{'>'}</code> 및 OpenGraph 메타 태그를 생성하여 소셜 공유 미리보기 카드에 실시간 바인딩합니다.</p>
                </div>

                <div>
                  <h5 className="font-bold text-zinc-900 dark:text-zinc-100 mb-1">3. 실무적 장점 (Why Use This)</h5>
                  <ul className="list-disc list-inside space-y-1 text-zinc-600 dark:text-zinc-400 pl-1">
                    <li><strong>검색엔진 최적화(SEO) 및 검색 랭킹 향상</strong>: 크롤러가 HTML을 수신하는 즉시 완벽한 메타데이터와 구조화된 정보를 파싱합니다.</li>
