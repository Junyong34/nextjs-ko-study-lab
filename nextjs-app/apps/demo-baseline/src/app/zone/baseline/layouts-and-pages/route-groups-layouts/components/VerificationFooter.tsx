...
export function VerificationFooter(props: VerificationFooterProps = {}) {
  ...
  const defaultExpected = "• Route Groups를 활용한 다중 루트 레이아웃 분리 사양에 따른 정상 동작 및 상태 변화 관찰"
      ...
      />
              ...
              <div className="space-y-3.5 text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
                <div>
                  ...
                  <p>Route Groups(<code>(groupName)</code>)는 괄호로 폴더명을 감싸 URL 경로 구조에 영향을 주지 않으면서 라우트들을 논리적으로 그룹화하고, 쇼핑몰 고객 화면과 관리자/인증 화면에 서로 다른 루트 레이아웃(<code>(shop)/layout.tsx</code>, <code>(auth)/layout.tsx</code>)을 독립적으로 적용하는 표준 파일 컨벤션입니다.</p>
                </div>

                <div>
                  <h5 className="font-bold text-zinc-900 dark:text-zinc-100 mb-1">2. 데모 예제 기반 동작 원리</h5>
                  <p>본 데모에서는 GNB와 푸터가 포함된 일반 쇼핑몰 레이아웃(<code>(shop)</code>)과, 헤더/푸터가 제거된 독립적인 풀스크린 로그인 레이아웃(<code>(auth)</code>) 간의 라우팅 전환 시 루트 레이아웃 자체가 완전히 분리되어 적용되는 아키텍처를 검증합니다.</p>
                </div>
