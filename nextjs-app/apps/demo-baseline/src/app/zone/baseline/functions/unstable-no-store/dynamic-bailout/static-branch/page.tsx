...
export const metadata: Metadata = getDemoMetadata(
  'baseline',
  'functions/unstable-no-store/dynamic-bailout/static-branch',
)
...
export default function StaticBranchPage() {
  // unstable_noStore()를 호출하지 않는다 — Request-time API를 전혀 쓰지 않으므로
  ...
  return (
    <div className="space-y-2.5">
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        이 브랜치는 unstable_noStore()를 호출하지 않아 정적 prerender를 막는 지점이 없다.
      </p>
      <ParticipantProbeCard mode="static" participants={participants} renderedAt={renderedAt} />
    </div>
  )
}
