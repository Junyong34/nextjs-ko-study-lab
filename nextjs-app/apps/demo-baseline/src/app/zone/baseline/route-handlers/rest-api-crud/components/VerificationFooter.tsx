      ...
      />
              ...
              <div className="space-y-3.5 text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
                <div>
                  ...
                  <p><code>app/api/.../route.ts</code> 파일 컨벤션은 Web 표준 <code>Request</code> 및 <code>Response</code> 객체를 기반으로 HTTP 메서드(<code>GET</code>, <code>POST</code>, <code>PATCH</code>, <code>DELETE</code>)를 명시적으로 export하여 RESTful API 엔드포인트를 구축하는 App Router 표준 서버리스 API 스펙입니다.</p>
                </div>

                <div>
                  <h5 className="font-bold text-zinc-900 dark:text-zinc-100 mb-1">2. 데모 예제 기반 동작 원리</h5>
                  <p>본 데모에서는 상품 리소스에 대해 <code>GET</code>(목록/단건 조회), <code>POST</code>(신규 등록), <code>PATCH</code>(재고/가격 부분 수정), <code>DELETE</code>(삭제) 요청을 전송하고, <code>NextResponse.json()</code>을 통한 상태 코드(200, 201, 204, 400) 및 JSON 응답 반환 과정을 실시간으로 검증합니다.</p>
                </div>
