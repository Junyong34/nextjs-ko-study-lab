...
import { VerificationFooter, SsePacket } from './VerificationFooter'
...
export function SseStreamClient() {
  ...
  const startStreaming = () => {
    ...
    setIsAborted(false)
    setStatusMessage('● SSE 파이프라인 연결됨: 서버에서 700ms 텔레메트리 청크 스트리밍 수신 중...')
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (data.status === 'completed') {
          setStatusMessage('✔ 스트리밍 완료 (총 6개 패킷 전송 성공)')
          setIsCompleted(true)
        ...
        <div className="overflow-hidden rounded-md border border-zinc-800 bg-zinc-950 text-white shadow-md">
          ...
          <div className="max-h-64 overflow-y-auto p-4 font-mono text-xs space-y-2">
            ...
            {packets.map((pkt, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="rounded bg-emerald-950 px-1.5 py-0.5 text-[10px] text-emerald-400 font-bold border border-emerald-800">
                    #{pkt.step || 'END'}
                  </span>
                  <span>{pkt.message}</span>
                </div>
                {pkt.serverCpu !== undefined && (
