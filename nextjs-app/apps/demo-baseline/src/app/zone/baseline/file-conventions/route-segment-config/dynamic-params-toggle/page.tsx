  ...
  return (
    <DemoContainer className="space-y-6">
      <DemoGuideCard
        ...
        concept={`generateStaticParams()가 반환하지 않은 다이나믹 세그먼트(${UNKNOWN_PRODUCT_ID})에 접근했을 때, dynamicParams=true(기본값)면 요청 시점에 온디맨드로 서버 렌더링되고, false면 실제 404가 반환됩니다. 이 두 동작은 런타임 토글이 아니라 서로 다른 서브 라우트로 만들어야 대조할 수 있습니다.`}
        steps={[
          {
            step: 1,
            title: `[사전 생성됨: ${KNOWN_PRODUCT_IDS[0]}] 링크로 두 브랜치 모두 진입`,
            description: 'generateStaticParams에 포함된 ID는 dynamicParams 설정과 무관하게 두 브랜치 모두 정상 렌더링됩니다.',
            actionBadge: '기준선 확인',
          },
          {
            step: 2,
            title: `[사전 생성 안 됨: ${UNKNOWN_PRODUCT_ID}] 링크로 on-demand 브랜치 진입`,
            description: 'dynamicParams=true 라우트는 목록에 없는 ID도 요청 시점에 SSR로 렌더링합니다.',
            actionBadge: 'on-demand 확인',
          },
          {
            step: 3,
            title: `같은 [사전 생성 안 됨: ${UNKNOWN_PRODUCT_ID}] 링크로 blocked 브랜치 진입`,
