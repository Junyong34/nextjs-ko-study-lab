...
export const metadata: Metadata = getDemoMetadata(
  'baseline',
  'file-conventions/forbidden/admin-role-403/settlement-vault',
)
...
// 이 세그먼트에는 같은 폴더에 전용 forbidden.tsx가 있다(./forbidden.tsx).
// role!=admin일 때 forbidden()이 던지는 예외는 상위로 전파되지 않고
// 가장 가까운 이 파일이 상위 admin-role-403/forbidden.tsx보다 먼저 렌더링된다.
export default async function SettlementVaultPage() {
    ...
    <DemoContainer className="space-y-4">
      <div className="space-y-4 rounded-lg border border-emerald-200 bg-emerald-50 p-5 text-sm dark:border-emerald-900/50 dark:bg-emerald-950/20">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-emerald-200 pb-3 dark:border-emerald-900/40">
          <div>
            <h4 className="font-bold text-emerald-800 dark:text-emerald-300">정산 금고 (민감 정산 데이터)</h4>
            <p className="text-xs text-emerald-700/80 dark:text-emerald-400/80">
              세션 역할이 ADMIN이므로 forbidden()이 호출되지 않고 실제 정산 데이터가 렌더링되었습니다. 이 폴더에는 전용
              forbidden.tsx가 있다는 점을 기억해 두세요.
            </p>
          </div>
          <Link
            href={BASE_PATH}
