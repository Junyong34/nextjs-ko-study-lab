...
export default function TransientOutageError({
  error,
  reset,
  retry,
}: {
  ...
  useEffect(() => {
    ...
    // error.message가 아니라 서버 카운터를 직접 조회한다 — 프로덕션에서는
    // 서버 컴포넌트 에러 메시지가 일반 메시지로 대체되므로 이 방식이 더 정확하다.
    peekOrderStatusAction('transient').then(setStatus)
  }, [error])
    ...
    <div className="space-y-4 rounded-lg border-2 border-rose-500/40 bg-rose-50/40 p-5 dark:border-rose-900/50 dark:bg-rose-950/20">
      <div className="flex items-center justify-between border-b border-rose-200 pb-3 dark:border-rose-900">
        <h4 className="font-bold text-sm text-rose-900 dark:text-rose-200">
          주문 {config.orderId} 조회 실패 (order/transient-outage/error.tsx)
        </h4>
      ...
      <div className="rounded border border-zinc-200 bg-zinc-50 p-3 text-xs font-mono text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-300">
        서버 실제 attempt 카운터: {status ? status.attempt : '조회 중...'} / 회복 기준: {config.recoverAfter}회
      </div>
      ...
      {lastAction && (
        <p className="text-[11px] text-zinc-500">
          방금 {lastAction === 'retry' ? 'retry()' : 'reset()'}를 호출했습니다. 위 attempt 카운터가{' '}
          {lastAction === 'retry'
            ? '증가했다면 실제로 세그먼트가 재요청된 것'
            : '그대로라면 재요청 없이 에러 상태만 초기화된 것'}
          입니다.
        </p>
      )}
      <OrderVerificationPanel scenario="transient" phase="fail" attempt={status?.attempt} />
    </div>
  )
}
