...
export function VerificationFooter({
  ...
  const expected =
    '• 1단계: 1500ms 추천 상품 청크 수신 (3건)\n• 2단계: 3000ms 구매 후기 중첩 청크 수신 (2건, 별점 ★★★★★ 정상 렌더링)\n• 계층적 중첩 Suspense 점진적 청크 스트리밍 정상 완료'

  const actual =
    !recommendedLoaded && !reviewsLoaded
      ? '• 청크 스트리밍 대기 중 (초기 상품 셸 수신 중...)'
      : recommendedLoaded && !reviewsLoaded
      ? '• 1차 외측 청크 수신 완료: 추천 상품 3건 마운트 (1500ms)\n• 2차 내측 청크 대기 중: 구매 후기 스트리밍 진행 중 (3000ms)...'
      ...
      <DemoDeepDiveCard title="중첩 Suspense 점진적 청크 스트리밍">
        <div className="space-y-3.5 text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
          ...
          <div>
            ...
            <p>
              본 데모에서는 즉시 표시되는 메인 상품 헤더 외에, 외측 Suspense(1500ms 소요 추천 상품) 내부에 내측 Suspense(3000ms 소요 구매 후기)를 중첩 배치하여, 외측 청크 도착 시 추천 상품과 내측 스켈레톤이 먼저 렌더링되고 이후 후기 데이터가 순차 교체되는 계층적 멀티 청크 스트리밍을 시각화합니다.
            </p>
          </div>
