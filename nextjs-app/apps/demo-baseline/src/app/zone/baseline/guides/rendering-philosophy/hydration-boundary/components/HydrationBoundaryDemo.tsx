...
export function HydrationBoundaryDemo({ onMountedChange }: HydrationBoundaryDemoProps) {
  ...
  // 서버 렌더링 시점에는 실행되지 않고, 브라우저에서 Hydration이 완료된 뒤에만 실행된다.
  // 이것이 실제 Next.js/React의 "마운트 게이트" 패턴이다. 버튼 클릭이 아니라 Hydration 자체가 트리거다.
  useEffect(() => {
  ...
  return (
    <div className="rounded border border-zinc-200 bg-white p-4 shadow-2xs dark:border-zinc-800 dark:bg-zinc-950 space-y-2">
      <div className="text-xs font-bold text-zinc-900 dark:text-zinc-100">
        Hydration 상태: {mounted ? '[확인] 클라이언트 Hydration 완료 (자동 감지됨)' : '서버 HTML 렌더 (브라우저 전용 값 표시 불가)'}
      </div>
      <p className="text-xs text-zinc-500">
        이 상태는 버튼 클릭이 아니라 <code>useEffect(() =&gt; setMounted(true), [])</code>가 Hydration 뒤 자동 실행되어 전환됩니다.
        서버는 <code>window</code>에 접근할 수 없으므로, 마운트 전에는 브라우저 전용 값(뷰포트 크기 등)을 렌더링하지 않아야 Hydration 불일치를 줄일 수 있습니다.
      </p>
    </div>
  )
}
