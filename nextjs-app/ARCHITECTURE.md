...
| [`packages/demo-kit`](./packages/demo-kit) | `@study/demo-kit` | - | • **데모 존 공통 UI 키트**<br>• `DemoContainer`: 내부 DOM 높이를 실시간 측정하여 부모 셸에 `DEMO_RESIZE` postMessage 전송<br>• `DemoGuideCard`/`DemoPlaygroundCard`/`DemoDeepDiveCard`: 모든 데모가 따르는 4단 표준 레이아웃(가이드→실습→검증→개념 정리)의 축 (`AGENTS.md` 규칙 25)<br>• `ExpectedActualPanel`: 기대값과 실제 관찰 상태를 비교하는 검증 배지 패널<br>• `DemoResetButton`: 데모 상태 초기화 버튼<br>• `ecommerce/`: 이커머스 도메인 데모 전용 `ProductCard`/`CartSummary`/`DeliveryTracker`/mock 데이터 (ADR 0007)<br>• 데모 앱에는 shadcn을 넣지 않는다 — 학습자가 읽을 코드다 |
