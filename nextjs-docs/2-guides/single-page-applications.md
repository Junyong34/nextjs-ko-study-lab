...
[SWR](./2.15-client-side-data-fetching/swr.md)는 클라이언트의 focus·interval 재검증, mutation, 컴포넌트 간 요청 중복 제거가 필요할 때 사용한다. SWR 2.3.0과 React 19 이상에서는 기존 `useSWR` 코드를 유지하면서 Server Component가 첫 데이터를 제공할 수 있다.

- **클라이언트 전용**: `useSWR(key, fetcher)`
- **서버 전용**: `useSWR(key)`와 RSC 제공 데이터
- **혼합**: `useSWR(key, fetcher)`와 RSC 제공 데이터

Server Component의 `<SWRConfig>`에 기다리지 않은 Promise를 `fallback`으로 넘기면 해당 키를 읽는 하위 컴포넌트만 suspend한다. 서버에서는 쿠키·헤더·데이터베이스를 직접 읽을 수 있어 별도 API route가 필요하지 않다.
...
Client Component는 같은 키로 기존 `useSWR` 호출을 유지한다. 첫 렌더링에는 서버가 제공한 데이터가 있으므로 `data`가 `undefined`인 상태를 별도로 처리할 필요가 없고 `isLoading`은 `false`다. 이후 polling·재검증·캐싱은 클라이언트에서 계속 동작한다.

| 기능 | SWR | RSC | RSC + SWR |
| --- | --- | --- | --- |
| SSR 데이터 | 지원 안 함 | 지원 | 지원 |
| SSR 중 스트리밍 | 지원 안 함 | 지원 | 지원 |
| 요청 중복 제거 | 지원 | 지원 | 지원 |
| 클라이언트 기능 | 지원 | 지원 안 함 | 지원 |

##### 데이터를 사용하는 컴포넌트 범위로 서버 데이터 제한하기

`<SWRConfig>`는 root layout뿐 아니라 데이터를 소유한 route segment에도 둘 수 있다. page 수준 설정은 부모 설정의 `fallback` 키와 병합되므로 무관한 데이터를 전역 설정에 넣지 않고, 다이나믹 route의 `params`를 `<Suspense>` 안에서 푼 뒤 해당 하위 트리의 요청만 시작할 수 있다.

```tsx
function ProjectData({ id }: { id: string }) {
  return (
    <SWRConfig value={{ fallback: { [`/api/projects/${id}`]: getProject(id) } }}>
      <ProjectView id={id} />
    </SWRConfig>
  )
}
...
> **알아두면 좋은 점**: `fallback` 키와 `useSWR` 키는 정확히 같아야 한다. 불일치해도 경고가 나오지 않고 서버 데이터가 읽히지 않아 클라이언트에서 다시 요청한다. route params나 search params 같은 다이나믹 값으로 키를 만들 때는 서버와 클라이언트가 어긋나지 않도록 하나의 공유 함수나 상수에서 생성한다. `fallback`은 첫 렌더링만 시드하므로 재검증 때도 요청을 재사용하려면 SWR의 `preload`를 사용한다.

#### TanStack Query를 사용하는 SPA

[TanStack Query](./2.15-client-side-data-fetching/tanstack-query.md)도 Server Component에서 cache를 시드하고 클라이언트가 재검증을 이어갈 수 있다. 서버에서는 요청마다 새 Query Client를 만들고 브라우저에서는 singleton을 사용한다. Server Component가 `prefetchQuery`를 기다리지 않고 시작한 다음 `dehydrate(queryClient)` 결과를 `<HydrationBoundary>`로 전달한다.
...
Client Component는 서버 prefetch와 같은 query key를 사용한다. `<Suspense>`가 loading을 처리하고 `data`가 항상 정의되기를 원하면 `useSuspenseQuery`, 컴포넌트 안에서 `isPending`과 오류를 직접 렌더링하려면 `useQuery`를 선택한다.

> **알아두면 좋은 점**: [Cache Components](../1-getting-started/caching.md)로 데이터를 캐시할 때는 [`"use cache"`](../3-api-reference/3.4-directives/use-cache.md)를 `getProject` 같은 데이터 함수에 추가하고 `dehydrate()` 결과 자체를 캐시하지 않는다. dehydrated state를 캐시하면 timestamp 같은 TanStack Query metadata까지 재사용되어 이후 요청에 오래된 데이터를 제공할 수 있다.
