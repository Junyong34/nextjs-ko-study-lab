...
> - Server Component에서 쿼리 파라미터에 따라 데이터를 가져오려면 Page 컴포넌트의 [`searchParams` prop](../3.1-file-conventions/page.md#searchparams-optional)을 읽는 것이 권장된다.
> - 애플리케이션에 `/pages` 디렉토리가 공존하는 경우 마이그레이션 호환성을 위해 `ReadonlyURLSearchParams | null`을 반환할 수 있다.

### 동작 방식 (Behavior)

#### Prerendering 및 `Suspense` 경계
...
따라서 정적 페이지에서 `useSearchParams`를 사용하는 Client Component는 반드시 `<Suspense>` 경계로 감싸야 한다. 그렇지 않으면 프로덕션 빌드 시 에러가 발생한다.
...
라우트가 다이나믹 렌더링되는 경우, `useSearchParams`는 초기 서버 렌더링 시점에도 서버에서 바로 사용 가능하다. Next.js 15+에서는 요청 시점에 실행을 묶기 위해 Server Component에서 [`connection()`](./connection.md)을 호출하는 것을 권장한다.
...
정답: **B**  
해설: 정적으로 prerender되는 페이지에서 `useSearchParams`를 사용할 경우, 가장 가까운 `Suspense` 경계까지 클라이언트 측 렌더링으로 전환되므로 빌드 에러를 방지하고 fallback을 제공하기 위해 `<Suspense>` 경계로 감싸야 한다.
</details>
...
- 정적 prerender 환경에서는 빌드 오류 방지 및 원활한 렌더링을 위해 `<Suspense>` 경계로 감싸야 한다.
- 다이나믹 렌더링 환경(`connection()` 호출 등)에서는 초기 서버 렌더링 시에도 즉시 값을 사용할 수 있다.
- Server Component에서는 Page의 `searchParams` prop을 사용하며, Layout은 쿼리 파라미터를 직접 전달받지 않는다.
