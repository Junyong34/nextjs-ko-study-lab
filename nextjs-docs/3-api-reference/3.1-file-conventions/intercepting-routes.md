...
![경로 소프트 네비게이션 차단](./assets/intercepting-routes-01.webp)

그러나 공유 가능한 URL을 클릭하거나 페이지를 새로 고쳐 사진으로 이동하면 모달 대신 전체 사진 페이지가 렌더링되어야 한다. 라우트 가로채기이 발생해서는 안 된다.
...
라우트 가로채기은 `(..)` 규칙으로 정의할 수 있다. 이는 상대 경로 규칙 `../`와 유사하지만 라우트 세그먼트에 대한 것이다.
...
위의 예에서 `@modal`는 세그먼트가 **아님** 슬롯이므로 `photo` 세그먼트에 대한 경로는 `(..)` 일치자를 사용할 수 있다. 이는 파일 시스템 수준이 두 개 더 높음에도 불구하고 `photo` 경로가 세그먼트 수준 한 개만 높다는 것을 의미한다.
