...
`opengraph-image`와 `twitter-image`는 공유 링크에 표시할 image를 segment별로 설정한다. 정적 파일은 `.jpg|jpeg|png|gif`를 지원하며 같은 이름의 `.alt.txt`로 대체 텍스트를 제공한다. Twitter image는 5MB, Open Graph image는 8MB를 넘으면 build가 실패한다.
...
정적 파일과 코드 variant는 현재 segment부터 가장 가까운 파일이 하위 route에 적용되는 방식으로 계층화된다. 코드 생성은 기본적으로 build 때 최적화되지만 Request-time API나 uncached dependency를 사용하면 요청 시점에 생성된다.

## 예제 및 데모 설계

- Phase 2에서 blog slug별 Open Graph image와 alt를 생성하고 meta tag를 검사한다.
- 제한을 넘는 파일의 build 오류와 여러 image의 정렬을 확인한다.
