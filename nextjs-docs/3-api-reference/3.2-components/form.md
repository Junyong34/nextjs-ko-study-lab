...
- `next/form`의 `Form` 컴포넌트가 일반 `<form>`을 확장해 제공하는 세 가지 기능(로딩 UI 프리페치, 클라이언트 사이드 내비게이션, progressive enhancement)을 설명한다.
...
- **prefetching**: `Form`이 뷰포트에 보이면 `loading.js` 같은 로딩 UI를 미리 [프리페치](../../1-getting-started/linking-and-navigating.md#prefetching)한다.
...
- **`prefetch`**: 뷰포트에 폼이 보일 때 경로를 프리페치할지 정한다. 기본값은 `true`다.
...
- `onSubmit`을 사용해 제출을 처리할 수 있지만, `event.preventDefault()`를 호출하면 문자열 `action`으로 지정한 경로로 이동하는 `Form`의 프리페치·내비게이션 동작이 무효화된다.
...
`Form`이 보이는 순간 `SearchPage`용 [`loading.js`](../3.1-file-conventions/loading.md) 파일이 프리페치되므로, 사용자가 폼을 제출하면 새 페이지로 이동하는 동안 로딩 UI가 즉시 나타난다.
...
프리페치되지 않는 경우를 대비해, [`useFormStatus`](https://react.dev/reference/react-dom/hooks/useFormStatus)로 제출 중 대기 상태를 보여줄 수도 있다.
...
> **알아두면 좋은 점**: `action`이 함수일 때는 실제로 어떤 요청이 갈지 사전에 알 수 없으므로, `Form`은 대상 경로를 프리페치하지 않는다.
...
- `loading.js`를 배치해 `Form`이 뷰포트에 들어올 때 프리페치가 일어나는지, 제출 시 로딩 UI가 즉시 나타나는지 비교한다.
- `useFormStatus`로 제출 대기 상태를 표시하는 버튼을 구현해, 프리페치가 되지 않는 경우의 대체 UX를 확인한다.
...
1. `<Form action="/search">`와 `<Form action={createPost}>`의 동작 차이로 옳은 것은?
   ...
   문자열 `action`은 GET 방식 폼 제출처럼 동작해 폼 데이터를 쿼리 파라미터로 URL에 인코딩하고, 이동 전에 대상 경로를 프리페치한다. 함수 `action`은 React의 form action(Server Function)으로 동작하며, 어떤 요청이 갈지 미리 알 수 없어 프리페치되지 않는다.
...
3. `Form`에서 `onSubmit` 핸들러 안에서 `event.preventDefault()`를 호출하면 어떤 문제가 생길 수 있는가?
   ...
   `preventDefault()`가 기본 제출 동작을 막으면, 문자열 `action`으로 지정한 경로로 이동하는 `Form`의 프리페치·클라이언트 사이드 내비게이션 동작이 함께 무효화된다.

   </details>

## 챕터 요약

- `Form`은 `<form>`을 확장해 프리페치, 클라이언트 사이드 내비게이션, progressive enhancement를 제공한다.
