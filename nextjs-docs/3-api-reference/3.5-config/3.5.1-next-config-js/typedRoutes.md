...
- `<Link>` 컴포넌트 및 `useRouter().push()`에서 오타 방지 및 자동 완성, 다이나믹 라우트 검증 기법을 설명할 수 있다.
...
과거 실험적 옵션(`experimental.typedRoutes`)으로 제공되던 기능이 정식 안정화(stable)되어, 별도의 외부 플러그인 없이도 오타로 인한 잘못된 링크 이동을 사전에 방지할 수 있다.
...
2. **선언 파일 생성**: 탐색된 경로들을 유니온 타입(`Route<T>`, `RouteImpl<T>`)으로 묶어 `.next/types/link.d.ts` 파일에 생성한다.
3. **컴파일러 타입 검증**: 프로젝트의 `tsconfig.json`이 `.next/types/**/*.ts`를 참조하여 편집기(IDE) 내 자동 완성 및 `tsc` 빌드 검증에 반영한다.
...
다이나믹 세그먼트를 포함하는 다이나믹 라우트(예: `/blog/[slug]`)의 경우 템플릿 리터럴 문자열을 작성하거나 명시적인 URL 객체 형태로 전달할 수 있다:
...
> - **`tsconfig.json` include 확인**: Next.js가 자동 생성하는 타입 선언을 인식하기 위해 `tsconfig.json`의 `include` 배열에 `.next/types/**/*.ts`가 등록되어 있어야 한다 (`next dev` 실행 시 자동으로 구성된다).
> - **`next typegen` CLI 지원**: 전체 애플리케이션 빌드를 수행하지 않고도 CI 파이프라인에서 라우트 타입만 생성하여 타입 체킹(`next typegen && tsc --noEmit`)을 수행할 수 있다.
...
- `typedRoutes`는 개발 및 빌드 단계의 정적 분석 도구로, 브라우저에서 동적으로 변화하는 UI 동작을 가지지 않는다.
- VS Code 등 에디터에서 `<Link href="..." />` 작성 시 지원되는 인라인 자동 완성 목록과, 오타 발생 시 터미널 `next build` 과정에서 나타나는 타입 컴파일 오류(`Type '"/dashbord"' is not assignable to type 'RouteImpl<"/dashbord">'`)를 통해 기능을 검증한다.
...
- 다이나믹 라우트의 경우 `Route` 타입 단언 또는 템플릿 리터럴을 활용하여 안전하게 경로를 조합할 수 있다.
