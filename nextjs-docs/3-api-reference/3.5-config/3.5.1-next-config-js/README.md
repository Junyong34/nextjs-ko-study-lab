...
| 옵션명 | 설명 |
|---|---|
| [`basePath`](./basePath.md) | 애플리케이션을 도메인의 서브 경로(예: `/docs`) 하위에 배포할 때 지정 |
| [`assetPrefix`](./assetPrefix.md) | 정적 자산(JS/CSS/이미지)을 CDN 도메인에서 제공하도록 접두사 지정 |
| [`env`](./env.md) | 빌드 시점에 인라인 주입할 환경 변수 정의 |
| [`images`](./images.md) | `next/image`의 원격 도메인(`remotePatterns`), 디바이스 크기, 포맷 설정 |
| [`redirects`](./redirects.md) | URL 패턴 기반의 HTTP 리다이렉트(307/308) 규칙 배열 반환 |
| [`rewrites`](./rewrites.md) | 주소창 변경 없이 내부 경로 또는 외부 API로 프록시하는 재작성 규칙 |
| [`headers`](./headers.md) | 보안 및 캐싱을 위한 커스텀 HTTP 응답 헤더 등록 |
| [`output`](./output.md) | 배포 최적화 출력 모드 (`'standalone'`, `'export'`) |
| [`transpilePackages`](./transpilePackages.md) | 모노레포 패키지 또는 외부 ESM 라이브러리의 트랜스파일 대상 등록 |
| [`turbopack`](./turbopack.md) | Turbopack 번들러 전용 로더 및 모듈 앨리어스 설정 |
| [`typedRoutes`](./typedRoutes.md) | 정적 타이핑된 라우트 경로 지원 (`experimental`) |
...
개별 옵션은 아래 문서에서 다룬다. **65개의 독립적인 참조 항목**이라 학습 순서를 재배열하지 않고 공식 문서의 알파벳 순서를 그대로 유지한다.
