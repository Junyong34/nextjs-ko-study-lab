...
- React Strict Mode의 역할과 Next.js에서의 기본 활성화 동작 방식을 이해한다.
- 개발 환경에서의 컴포넌트 이중 호출(Double Invocation) 및 레거시 API 감지 원리를 파악한다.
- 전역 비활성화와 하위 트리(`<React.StrictMode>`) 단위의 점진적 도입 전략을 설명할 수 있다.

## 핵심 개념 및 설명

React의 Strict Mode는 개발 모드(`next dev`)에서 실행되는 개발용 런타임 검사 도구다. 애플리케이션 코드 내에서 레거시 API 사용, 안전하지 않은 생명주기(lifecycle) 메서드, 그리고 처리되지 않은 부작용(side effect)을 식별하여 React의 향후 업데이트 및 기능에 안정적으로 대비할 수 있도록 돕는다.

`next.config.js`, `next.config.mjs`, 또는 `next.config.ts` 파일의 `reactStrictMode` 설정을 통해 애플리케이션 전체의 Strict Mode 활성화 여부를 제어할 수 있다.
...
Strict Mode 검사는 개발 서버(`next dev`) 실행 중에만 동작한다. 프로덕션 빌드(`next build` 및 `next start`)에서는 완전히 제외되므로, 최종 사용자 환경의 성능이나 번들 크기에 전혀 영향을 주지 않는다.

#### 렌더링 및 Effect 이중 실행 (Double Invocation)

React는 렌더링 단계(render phase)가 순수 함수(pure function)처럼 동작해야 한다는 전제를 가진다. 부작용이 없어야 할 위치에서 발생하는 예기치 못한 사이드 이펙트나 리소스 누수를 사전에 발견하기 위해, 개발 모드에서 다음과 같은 동작을 의도적으로 두 번 실행한다:
...
오래된 서드파티 라이브러리를 사용하거나 레거시 코드가 많은 대규모 프로젝트에서는 Strict Mode를 전역 활성화했을 때 방대한 양의 경고가 발생할 수 있다. 이러한 경우 `next.config.ts`에서 전역 옵션을 끄고, 리팩토링이 완료된 특정 하위 컴포넌트 트리에만 `<React.StrictMode>` 컴포넌트를 감싸 점진적으로 적용할 수 있다:
...
> - **활성화 강력 권장**: 향후 React 기능과의 호환성을 유지하고 애플리케이션 품질을 높이기 위해 Next.js 프로젝트에서 Strict Mode를 활성화하는 것을 적극 권장한다.
...
- 프로덕션 빌드(`next build`)에는 번들 및 런타임에 아무런 영향을 미치지 않는다.
