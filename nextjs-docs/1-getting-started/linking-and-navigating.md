...
Next.js에서 라우트는 기본적으로 서버에서 렌더링된다. 이는 새 라우트를 보여주기 전에 클라이언트가 서버 응답을 기다려야 한다는 뜻이다. Next.js는 내장된 [prefetching](#prefetching), [스트리밍](#스트리밍), [클라이언트 사이드 전환](#클라이언트-사이드-전환)으로 내비게이션을 빠르고 반응성 있게 유지한다.
...
- **Prerendering**: 빌드 타임 또는 [revalidation](./revalidating.md) 중에 일어나고 결과가 캐시된다.
- **Dynamic Rendering**: 클라이언트 요청에 대응해 요청 시점에 일어난다.

서버 렌더링의 트레이드오프는 클라이언트가 새 라우트를 보기 전에 서버 응답을 기다려야 한다는 점이다. Next.js는 사용자가 방문할 가능성이 높은 라우트를 [prefetching](#prefetching)하고 [클라이언트 사이드 전환](#클라이언트-사이드-전환)을 수행해서 이 지연을 해결한다.

> **알아두면 좋은 점**: 최초 방문 시에도 HTML이 생성된다.

#### prefetching

prefetching은 사용자가 실제로 이동하기 **전에** 백그라운드에서 라우트를 미리 불러오는 과정이다. 사용자가 링크를 클릭할 때쯔음엔 다음 라우트를 렌더링할 데이터가 이미 클라이언트에 준비되어 있어, 라우트 간 이동이 즉각적으로 느껴진다.
...
- **다이나믹 라우트**: prefetching이 스킵되거나, [`loading.tsx`](../3-api-reference/3.1-file-conventions/loading.md)가 있으면 부분적으로 prefetching된다.
...
> **알아두면 좋은 점**: prefetching의 전체 동작, 링크별 제어 방법, [Partial Prefetching](../2-guides/adopting-partial-prefetching.md) 도입 시 어떻게 바뀌는지는 별도 가이드를 참고한다.
...
다이나믹 라우트에서는 공유 레이아웃과 로딩 스켈레톤을 미리 요청할 수 있어, **부분적으로 prefetching**될 수 있다는 뜻이다.
...
클라이언트 사이드 전환은 서버 렌더링 앱을 클라이언트 렌더링 앱처럼 **느껴지게** 만드는 핵심이다. [prefetching](#prefetching)과 [스트리밍](#스트리밍)이 함께 작동하면, 다이나믹 라우트에서도 빠른 전환이 가능해진다.
...
다이나믹 라우트에 `loading.tsx`를 추가해 부분 prefetching을 활성화하고, 즉각적인 내비게이션을 유발하고, 라우트가 렌더링되는 동안 로딩 UI를 보여주는 것을 권장한다.
...
> **알아두면 좋은 점**: **실험적인** [`useOffline`](../2-guides/offline-support.md) 훅을 쓰면, 연결이 끊긴 동안에도 prefetching된 라우트를 계속 이동할 수 있게 유지할 수 있다. [오프라인 지원 가이드](../2-guides/offline-support.md)를 참고한다.
...
- 예제에서 관찰할 결과: 정적 라우트는 뷰포트에 들어오는 즉시 전체가 prefetch되고, 다이나믹 라우트는 `loading.tsx` 유무에 따라 prefetching 범위가 달라지는 것.
...
- [ ] `loading.tsx`가 있으면 다이나믹 라우트도 부분적으로 prefetching될 수 있다.
...
1. `prefetch={false}`로 prefetching을 끈다.
...
- 정적 라우트는 뷰포트에 들어오면 전체가 prefetch되고, 다이나믹 라우트는 스킵되거나 `loading.tsx`가 있을 때만 부분 prefetching된다.
- `loading.tsx`는 내부적으로 `page.tsx`를 `<Suspense>`로 감싸 즉각적인 내비게이션과 로딩 UI를 제공한다.
- `generateStaticParams` 부재, 느린 네트워크, hydration 지연은 내비게이션을 느리게 느껴지게 만드는 흔한 원인이다.
